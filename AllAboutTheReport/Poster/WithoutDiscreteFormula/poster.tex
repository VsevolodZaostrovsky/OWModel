...
\begin{frame}[t]
    ...
    \eqref{optexp}. The first step of our research is implemeting the following fornula (that is a 
    limit of discrete solution which may be used for big $N$). 
    The next step is implementing of the discrete solution and considering thoose 
    strtegies.
    ...
    \begin{block}{The key question: how to find $\rho$?}
        ...
        \begin{itemize}
            \item $\Delta A_{k+2}$ is an ask change after execution of the limit order with the depth $x_k$.
            \item $\Delta t_{k+2}$ is a time time between two adjacent orders of dataset.
        \end{itemize}
